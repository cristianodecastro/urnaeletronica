void menu_operacional_aguardando_encerrada(char* menu_index, char tecla, char* option_menu, char urna_estado){
    // Menu do sistema
	switch(menu_index*){
		case 1:
			...
			else if(tecla == 'A'){ *option_menu = OPTION_ESTADO; *menu_index = 0;}
			break;
		case 2:
    		if(*option_menu == OPERACIONAL){
    			...
    			tecla = getCharacter_AB();
    			if(tecla == 'B'){ *menu_index++;}
    			else if(tecla == 'A'){ *option_menu = OPTION_ELEITOR; *menu_index = 0;}
    		} else{
    		    *menu_index++;
    		}
			break;
		case 3:
			...
			tecla = getCharacter_AB();
			if(tecla == 'B') { *menu_index++;}
			else if(tecla == 'A') { *option_menu = OPTION_CONSULTA_HORA; *menu_index = 0;}
			break;
		case 4:
			...
			tecla = getCharacter_AB();
			if(tecla == 'B') { *menu_index++;}
			else if(tecla == 'A') { *option_menu = OPTION_TROCA_HORA; *menu_index = 0;}
			break;
		case 5:
			...
			tecla = getCharacter_AB();
			if(tecla == 'B') { *menu_index++;}
			else if(tecla == 'A') { *option_menu = OPTION_VERIFICA_CORRESPONDENCIA; *menu_index = 0;}
			break;
		case 6:
			...
			tecla = getCharacter_AB();
			if(tecla == 'B') {*menu_index++;}
			else if(tecla == 'A') {*option_menu = OPTION_RELATORIO_VOTACAO; *menu_index = 0;}
			break;
		case 7:
			...
			tecla = getCharacter_AB();
			if(tecla == 'B') {*menu_index++;}
			else if(tecla == 'A') {*option_menu = OPTION_TROCA_SENHA; *menu_index = 0;}
			break;
		case 8:
			...
			else if(tecla == 'A') {*option_menu = OPTION_RESET_SENHA; *menu_index = 0;}
			break;
		default:
			break;
	}
...
void options_menu_operacional_aguardando_encerrada(char* menu_index, char* option_menu, char* urna_estado, char* numero_eleitor, char* nome_eleitor, char* num_eleitores, char* senha_mesario, CANDIDATO * depFederal, CANDIDATO * partido, CANDIDATO * senador, CANDIDATO * presidente, ELEITOR eleitores_que_votaram){

    char tecla;
    char resp[2];
    char validacao_eleitor;
    char char validacao_votacao;
    ...
    switch(*option_menu){
        ...
        case OPTION_ESTADO:
    		ver_estado(menu_index, option_menu, urna_estado);
    		break;

        // Novo eleitor
    	case OPTION_ELEITOR:
    	    if(*urna_estado == OPERACIONAL){
        		...
        		do{
        			...
        			if(tecla == 'A' && input_caracters_counter == 5){
        			    ...
        			    validacao_eleitor = valida_eleitor(numero_eleitor, nome_eleitor, *num_eleitores/*, eleitores_que_votaram*/);
        				...
        				if(validacao_eleitor == ELEITOR_VALIDO){
        					...
        					if(tecla == 'A'){
        						...
        						validacao_votacao = votacao(depFederal, partido, senador, presidente);
    	    ...
    	    case OPTION_CONSULTA_HORA:
        		consulta_hora(menu_index, option_menu);
        		break;

        	case OPTION_TROCA_HORA:
        		troca_hora(menu_index, option_menu);
        		break;

        	case OPTION_VERIFICA_CORRESPONDENCIA:
        		verifica_correspondencia(menu_index, option_menu, urna_estado, *num_eleitores);
        		break;
        	...
        	case OPTION_TROCA_SENHA:
        		...
        		else {*option_menu = 0; *menu_index = OPTION_TROCA_SENHA;}
        		break;
